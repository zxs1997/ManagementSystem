...
public class Controller_content {
    ...
    public void queryData(ActionEvent actionEvent) {
        switch (table_selected) {
            case stu_info:
                query_label.setText("查询姓名");
                break;
            case course_info:
                query_label.setText("查询课程名");
                break;
            default:
                break;
        }
    ...
    public void confirm_query(ActionEvent actionEvent) {
        switch (table_selected) {
            case stu_info:
                //此list用于装载数据
                ObservableList<Students> stu_list = FXCollections.observableArrayList();
                ...
                StudentsDAO studentsDAO = new StudentsDAO();
                String SQL = "SELECT * FROM stu_info WHERE stu_name LIKE'%" + query_textfield.getText() + "%';";
